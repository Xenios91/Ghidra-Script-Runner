...
func TestStartGhidraAnalysis(t *testing.T) {
	...
	dummyValues := "test"
	configuration := &config.Configuration{ClassificationDetailsFile: &dummyValues, GhidraHeadless: &dummyValues,
		GhidraProjectLocation: &dummyValues, GhidraProject: &dummyValues, GhidraScript: &dummyValues}
	LoadGhidraAnalysis(configuration)
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
